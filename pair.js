...
router.get('/', async (req, res) => {
    ...
    if (!phone.isValid()) {
        if (!res.headersSent) {
            return res.status(400).send({ code: 'Invalid phone number. Please enter your full international number (e.g., 447219076123 for UK, 27835515085 for South Africa, etc.) without + or spaces.' });
        }
        return;
    }
    // Use a safe international number format (prefer e164). Remove any '+' and non-digits.
    let formatted = phone.getNumber('e164') || phone.getNumber('international') || phone.getNumber();
    if (!formatted) {
        if (!res.headersSent) return res.status(400).send({ code: 'Unable to format phone number' });
        return;
    }
    formatted = String(formatted).replace('+', '').replace(/[^0-9]/g, '');
    num = formatted;
